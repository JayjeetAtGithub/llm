...
int main(int argc, char **argv) {
    ...
    if (operation == "index") {
        ...
        std::cout << "[INFO] adding vectors to index" << std::endl;
        ...
        std::cout << "[TIME] index: " << std::chrono::duration_cast<std::chrono::milliseconds>(e - s).count() << " ms" << std::endl;

        std::string hnsw_path = "index." + dataset + ".hnswlib";
        alg_hnsw->saveIndex(hnsw_path);
        std::cout << "[FILESIZE] index_size: " << filesize(hnsw_path.c_str()) << " bytes" << std::endl;
